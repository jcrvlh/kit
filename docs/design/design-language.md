...
| **Catálogo** | Instala/remove Tools por Wi-Fi (Home → VER TODOS → SISTEMA) | Titlebar `CATÁLOGO` + `lv_tileview` de 2 páginas: lista de Tools (barra `TOOLS X/32` + botão `ATUALIZAR TODAS (N)` quando há updates + selo `INSTALAR`/`ATUALIZAR`/`INSTALADA`) e **SOBRE O LIMITE** (explica o teto de 32 Tools do catálogo); ou aviso `IR PARA WI-FI` | Toque numa Tool → detalhe · arrasta pro lado → limite · Titlebar ← → Home |
