...
├── tool.so               # Objeto compartilhado Xtensa (dlopen no dispositivo)
...
O `manifest.json` carrega o campo `checksum` com o hash SHA-256 de `tool.so` e um
hash por asset (`tool.so`). A `signature.bin` assina os bytes canônicos do `manifest.json`,
...
4. **Integridade:** é calculada a soma SHA-256 de `tool.so` e de cada asset e
...
O hash de `tool.so` é reconferido a cada carregamento, antes da relocação em PSRAM.
