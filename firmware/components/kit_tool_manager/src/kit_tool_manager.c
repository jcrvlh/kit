...
kit_err_t kit_tool_manager_start(const char *tool_id)
{
    ...
    } else if (strcmp(tool_id, "com.kit.soundbox") == 0) {
        err = kit_soundbox_start(KIT_COLOR_GREEN); // cor do card "Soundbox" na Home
        primary_action = kit_soundbox_replay;      // PWR físico retoca o último pad
    } else {
...
void kit_tool_manager_stop_current(void)
{
    ...
    if (strcmp(s_current_tool, "com.kit.dice") == 0) {
        ...
        kit_placar_destroy();
    } else if (strcmp(s_current_tool, "com.kit.soundbox") == 0) {
        kit_soundbox_destroy();
