...
static int esp_elf_load_section(esp_elf_t *elf, const uint8_t *pbuf)
{
    ...
    /* KIT: elf->ptext é RAM interna executável (IRAM), que só aceita acesso
       alinhado de 32 bits no ESP32-S3. Se TEXT.size não é múltiplo de 4, este
       memcpy termina com um store sub-word (s8i/s16i) na IRAM -> LoadStoreError
       (a placa reinicia ao abrir uma Tool cujo .text tem tamanho ímpar em 4 —
       ex.: Quebra-Gelo 0xa86, Pavio 0x1d6b). O bloco já foi alocado com
       ELF_ALIGN(.., 4) e .text nunca é a última seção do arquivo, então
       arredondar a cópia lê no máximo 3 bytes a mais do pbuf (inofensivo) e
       mantém todos os stores alinhados. Não mexe em elf->sec[TEXT].size, que o
       esp_elf_map_sym usa para classificar endereços .text vs .rodata. */
