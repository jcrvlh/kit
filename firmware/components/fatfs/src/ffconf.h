...
/* KIT: com FF_FS_EXFAT ligado, ff.c usa FF_USE_LABEL como valor numa expressão
/  C (dir_read, código exFAT), não só em #if — e a ESP-IDF não emite símbolo
/  para um bool Kconfig "not set". Só este; NÃO definir CONFIG_FATFS_USE_FASTSEEK
/  nem _USE_DYN_BUFFERS aqui — vfs_fat.c os testa com #ifdef. */
...
/
/  KIT: componente fatfs copiado da ESP-IDF só para ligar exFAT. Motivo:
/  cartões microSD de 64 GB+ vêm em exFAT e o macOS não monta bem o FAT32 que
/  o f_mkfs gera. LBA de 64 bits (FF_LBA64) fica desligado — não precisa até
/  2 TB. Ver docs/hardware/microsd.md. Manter em sincronia com a ESP-IDF. */
